...
#    context.items = frappe.db.sql("select name, cargo_ref, container_no, container_size, status, yard_slot, yard_section from `tabYard Settings`", as_dict=True)
    ...
    context.schedule = frappe.db.sql("SELECT name, vessel, voyage_no, vessel_type, agents, eta_date, etd_date, status FROM `tabBooking Request` WHERE DATE(eta_date) > '2019-01-01' ORDER BY etd_date DESC", as_dict=True)
