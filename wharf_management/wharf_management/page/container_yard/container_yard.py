...
def get_all_items():
    ...
    items = frappe.db.sql("""SELECT DISTINCT `tabYard Settings`.name, `tabCargo`.status, `tabCargo`.container_content,`tabYard Settings`.yard_slot, `tabYard Settings`.yard_section, `tabCargo`.cargo_type,
        ...
        LEFT JOIN `tabCargo` ON `tabYard Settings`.yard_slot = `tabCargo`.yard_slot AND `tabCargo`.cargo_type NOT IN ("Vehicles","Heavy Vehicles") 
        ORDER BY `tabYard Settings`.yard_slot""", as_dict=True)
    return items

@frappe.whitelist()
def get_container(container_no):
    items = []

    items = frappe.db.sql("""SELECT DISTINCT `tabYard Settings`.name, `tabCargo`.status, `tabCargo`.container_content,`tabYard Settings`.yard_slot, `tabYard Settings`.yard_section, `tabCargo`.cargo_type,
        `tabYard Settings`.yard_sub_section, `tabCargo`.name as cargo_ref, `tabCargo`.container_no, `tabCargo`.container_size, `tabCargo`.hazardous, `tabCargo`.chasis_no, `tabCargo`.cargo_condition
        FROM `tabYard Settings` 
        LEFT JOIN `tabCargo` ON `tabYard Settings`.yard_slot = `tabCargo`.yard_slot AND `tabCargo`.cargo_type NOT IN ("Vehicles","Heavy Vehicles") AND `tabCargo`.container_no=%s 
        ORDER BY `tabYard Settings`.yard_slot""",(container_no) , as_dict=True)
    return items
...
def get_bay_row_items(bay_row):
    ...
    items = frappe.db.sql("""SELECT DISTINCT `tabYard Settings`.name, `tabCargo`.status, `tabCargo`.container_content,`tabYard Settings`.yard_slot, `tabYard Settings`.yard_section, `tabCargo`.cargo_type,
        `tabYard Settings`.yard_sub_section, `tabCargo`.name as cargo_ref, `tabCargo`.container_no, `tabCargo`.container_size, `tabCargo`.hazardous, `tabCargo`.chasis_no, `tabCargo`.cargo_condition
        FROM `tabYard Settings` 
        LEFT JOIN `tabCargo` ON `tabYard Settings`.yard_slot = `tabCargo`.yard_slot AND `tabCargo`.cargo_type NOT IN ("Vehicles","Heavy Vehicles") WHERE `tabYard Settings`.yard_sub_section=%s 
        ORDER BY `tabYard Settings`.yard_slot""",(bay_row) , as_dict=True)
    return items
...
def get_express_items():
    ...
    express_items = frappe.db.sql("""SELECT DISTINCT `tabYard Settings`.name, `tabCargo`.status, `tabCargo`.container_content,`tabYard Settings`.yard_slot, `tabYard Settings`.yard_section, `tabCargo`.cargo_type,
        ...
        ORDER BY `tabYard Settings`.yard_slot AND `tabCargo`.cargo_type NOT IN ("Vehicles","Heavy Vehicles") """ , as_dict=True)
