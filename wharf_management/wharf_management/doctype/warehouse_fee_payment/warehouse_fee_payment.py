...
class WarehouseFeePayment(Document):
	...
	def insert_fees(self):
		if self.cargo_type == 'Loose Cargo':
				if self.weight < self.volume: strqty = float(self.storage_days_charged * self.volume)
				if self.weight > self.volume: strqty = float(self.storage_days_charged * self.weight)
