...
#				from `tabCargo` where cargo_type = "Container" and booking_ref = %s and final_status in ("Discharged","Loading") group by work_type, container_content, container_size""", (self.booking_ref), as_dict=1)
			manifest_summary_table = frappe.db.sql("""select cargo_type, work_type, container_size, container_content, sum(handling_fee) as handling_fee, sum(storage_fee) as storage_fee, sum(wharfage_fee) as wharfage_fee, count(name) as number from `tabCargo` 
		...
		def get_bbulks_summary_list(self):
    			condition = ""
			bbulks_summary_table = frappe.db.sql("""select cargo_type, work_type, sum(net_weight) as weight, sum(volume) as volume, sum(handling_fee) as handling_fee, sum(storage_fee) as storage_fee, sum(wharfage_fee) as wharfage_fee
