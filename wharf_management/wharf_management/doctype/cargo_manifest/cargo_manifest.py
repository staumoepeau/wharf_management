...
#			frappe.db.sql("""Update `tabCargo` set status='Unknown' where parent = %s""", (self.name))
		...
		def get_manifest(self):
			...
			manifest_entries = frappe.db.sql("""SELECT name as cargo_refrence, container_no, bol, cargo_type, chasis_no, mark,
			work_type, container_size, container_content, status, voyage_no, booking_ref, storage_fee, handling_fee, manifest_check 
			FROM `tabCargo` 
			WHERE booking_ref = %s 
			AND docstatus = 1
			AND manifest_check = 'Confirm'""",(self.booking_ref), as_dict=1)
...
#			entries = sorted(list(manifest_summary_table))
		...
		def get_bbulks_summary_list(self):
			...
			bbulks_entries = frappe.db.sql("""select cargo_type, work_type, count(name) as number, sum(net_weight) as weight, sum(volume) as volume, sum(handling_fee) as handling_fee, sum(storage_fee) as storage_fee, sum(wharfage_fee) as wharfage_fee 
			from `tabCargo` where cargo_type in ("Break Bulk","Loose Cargo","Vehicles","Heavy Vehicles") and booking_ref = %s and manifest_check="Confirm" group by cargo_type, work_type and docstatus = 1""", (self.booking_ref), as_dict=1)
